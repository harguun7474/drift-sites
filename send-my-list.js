...
const csvFileName = args[0] || 'my-businesses.csv';
...
if (!fs.existsSync(csvPath)) {
  ...
  console.error('Please create the CSV file or specify the correct filename:');
  console.error('node send-my-list.js your-file.csv');
  process.exit(1);
}
...
console.log(`\nReady to send emails to ${businesses.length} businesses.`);
console.log('Press Ctrl+C to cancel or wait 5 seconds to continue...');

setTimeout(async () => {
  let sent = 0, failed = 0;
  for (const { name, email } of businesses) {
    if (!email || !name || email.toLowerCase() === 'email') continue; // Skip header row
    try {
